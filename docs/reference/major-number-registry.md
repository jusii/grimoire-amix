...
| 10 | `fdfs` | stock ✅ |
| 11 | `nfs` | stock ✅ |
| **12** | **`cdfs`** (this family) — appended last | claimed 🟡 — see note |

> 🟡 **Unresolved index.** Counting the shipped table's rows directly gives `cdfs` = **12** (with
> `INCLUDE_RFS` on, as shipped; 11 with it off) ✅ for the count itself. But
> `amix-cdfs/docs/packet-c-bench-results.md` records it as **index 11**, with stock as 0–10. The
> two differ by exactly the `rfs` row. **Resolve with the cdfs owner before promoting this row to
> ✅.** The safe statement, true either way: **`cdfs` is appended as the last `vfssw[]` row** ✅.
