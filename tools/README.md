...
| [`build-bootfloppy.sh`](build-bootfloppy.sh) | Build a **bootable** `boot.adf` from a donor bootstrap + your kernel ELF (`compress -b16` + patched `IBLK` descriptor); self-tests via the descriptor. | ✅ **boots in Amiberry** (reaches the root-disk prompt) |
...
The `boot.adf` format is **reverse-engineered** (Unix `compress`/LZW kernel → m68k ELF; `IBLK` boot
descriptor with length/size/checksum = folded byte-sum of the `.Z` — see
[`../docs/boot-disks/reverse-engineering-boot-adf.md`](../docs/boot-disks/reverse-engineering-boot-adf.md)).
`extract-kernel.sh` + `build-bootfloppy.sh` round-trip on the host, and a rebuilt floppy **boots in
Amiberry** to the original's root-disk prompt (no overrun, no warning). 🟡 A floppy carrying a
*driver-modified* kernel, and a full tape install, are the next validations. For an installed system,
the simplest path remains relinking into the on-disk **boot partition** with
